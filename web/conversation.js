  ...
  function renderTurns(){
    ...
    for(const turn of state.data.turns){
      ...
      if(turn.result){const r=turn.result;
        ...
        else answer.append(node('p','accessible-note',r.status==='no_sources'?'No supporting sources found.':r.status==='model_abstained'?'The local model could not establish an answer from the supplied sources.':'These are source passages, not an AI-generated answer.'));
