...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        chat=ConversationService(store);session=chat.create(keys['reader'],{'title':'Example: equipment context'})
        for i,q in enumerate(('What is the equipment collection status?','Who owns that?')):
            chat.submit(keys['reader'],session['id'],{'question':q,'mode':'sources','follow_up':bool(i),'request_id':'example-'+str(i),'after':i});chat.process_one()
        chat_data=chat.view(keys['reader'],session['id']);chat_list=chat.listing(keys['reader'])
        data={'conversation':chat_data,'conversation_list':chat_list,'desk':desk,'knowledge':knowledge,'notes':notes,'questions':questions,'pulse':pulse.view(keys['reader'])}
    additions=r'''
  if(path==='/desk/conversations' && data===undefined)return JSON.parse(JSON.stringify(PREVIEW.conversation_list));
  if(path==='/desk/conversations/'+PREVIEW.conversation.id && data===undefined)return JSON.parse(JSON.stringify(PREVIEW.conversation));
...
'''
    ...
    names=('app','knowledge','ask','os','conversation');html=(ROOT/'web/index.html').read_text()
    for name in names:
    ...
    raw=html.encode();target=out/'ALFRED-OS-v07.html';target.write_bytes(raw)
    (out/'manifest.json').write_text(json.dumps({'file':target.name,'bytes':len(raw),'sha256':hashlib.sha256(raw).hexdigest(),'mode':'actual frontend / fictional read-only transport','example_questions':EXAMPLES,'model_inference':False,'conversation_mode':'precomputed actual source retrieval, read-only','credentials_included':False},indent=2)+'\n')
