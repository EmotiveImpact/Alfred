...
def main():
    ...
    with sync_playwright() as p:
        ...
        preview=ROOT/'docs/previews/conversation-v07/ALFRED-OS-v07.html'
        ...
        page.locator('[data-view=conversation]').click();expect(page.locator('.conversation-turn')).to_have_count(2);check('example thread uses actual source retrieval');expect(page.locator('#conversation-status')).to_contain_text('read-only preview');expect(page.locator('#conversation-send')).to_be_disabled();expect(page.locator('#conversation-new')).to_be_disabled();expect(page.locator('#conversation-forget')).to_be_disabled();check('preview cannot create or mutate conversations')
