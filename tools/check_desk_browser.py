...
with tempfile.TemporaryDirectory() as temp:
    ...
    try:
        with sync_playwright() as p:
            ...
            expect(page.locator('#workspace')).to_be_visible();check('owner signs in against real HTTP')
            ...
            page.locator('[data-view="approvals"]').click();expect(page.locator('.action-card')).to_contain_text(message);check('draft remains after reload')
            ...
            page.locator('[data-view="evidence"]').click();page.locator('#refresh').click()
            ...
            for view in ['activity','sources','settings','overview']:
                page.locator(f'[data-view="{view}"]').click();expect(page.locator('#view-content')).not_to_be_empty();check('navigation works: '+view)
            ...
            page.locator('[data-view="settings"]').click();page.get_by_role('button',name='Sign out / switch workspace',exact=True).click()
            expect(page.locator('#sign-in')).to_be_visible();check('sign-out clears the working view')
            page.fill('#access-key',keys['reader']);page.get_by_role('button',name='Open your workspace').click();expect(page.locator('#workspace')).to_be_visible()
            expect(page.locator('#pause')).to_be_disabled();check('reader pause control disabled')
            page.get_by_role('button',name='Inspect evidence').first.click();check('reader has no draft preparation action',page.get_by_role('button',name='Prepare a response draft').count()==0)
            page.keyboard.press('Escape');store.revoke('demo-reader');page.locator('#refresh').click();expect(page.locator('#sign-in')).to_be_visible();check('credential revocation removes browser access without restart')
